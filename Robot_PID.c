...
////////////////////////////////////////////
// LFM - Line Following Module (CON PID)
//
// Este módulo implementa un controlador PID completo para seguir
// una línea negra en el suelo. Output speeds are stored in
...
// Constantes PID - Ajusta estos valores según tu robot
#define KP 0.4    // Ganancia Proporcional (era LFM_K_GS_SPEED)
#define KI 0.001  // Ganancia Integral (nueva)
#define KD 2.0    // Ganancia Derivativa (nueva)
...
void LineFollowingModule(void) {
  ...
  float P = KP * pid_error;
  
  // Término Integral (acumulación del error en el tiempo)
  ...
  float D = KD * pid_derivative;
  
  // Salida PID total
  pid_output = P + I + D;
  
  // Aplicar la corrección PID a las velocidades de las ruedas
  lfm_speed[LEFT] = LFM_FORWARD_SPEED - pid_output;
  lfm_speed[RIGHT] = LFM_FORWARD_SPEED + pid_output;
...
// Función para resetear el PID (útil al cambiar de modo o reiniciar)
void ResetPID(void) {
  pid_error = 0;
  ...
  if (llm_active) {  // Simply waiting until the line is not detected anymore
    if (side == LEFT) {
      if ((gs_value[GS_CENTER] + gs_value[GS_LEFT]) / 2 > LLM_THRESHOLD) {  // out of line
        ...
        lem_reset = TRUE;
        ResetPID();  // Resetear PID al salir de la línea
        // *** PUT YOUR CODE HERE ***
...
void LineEnteringModule(int side) {
  ...
  switch (lem_state) {
    ...
    case LEM_STATE_ON_LINE:
      ...
      ResetPID();  // Resetear PID al regresar a la línea
      break;
  }
}
...
int main() {
  ...
  for (;;) {  // Main loop
    ...
    if (Mode != wb_robot_get_mode()) {
      ...
      lem_active = FALSE;
      lem_state = LEM_STATE_STANDBY;
      ResetPID();  // Resetear PID al cambiar de modo
    ...
    // LFM - Line Following Module (AHORA CON PID)
    LineFollowingModule();
